...
### Business Description
Campaigns are collections of phone numbers to call using a specific agent — simpler than batch jobs.
...
| `POST` | `/campaigns/{id}/import_csv` | Import numbers from CSV |
| `POST` | `/campaigns/{id}/add_numbers` | Add numbers manually |
| `POST` | `/campaigns/{id}/start` | Start campaign execution |
| `GET` | `/campaigns/{id}/status` | Campaign overall status |

<Warning>
**Limitations:**
- Uses `BackgroundTasks` (not Celery) — no retry mechanism
- No campaign pausing or resuming
- Phone number deduplication on import but not on manual add
</Warning>
