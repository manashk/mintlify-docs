...
graph TB
    ...
    subgraph Core["Core Platform"]
        ...
        Dialer["Vaani Dialer<br/>Django + Twisted"]
    end

    subgraph Data["Data Layer"]
        PG["PostgreSQL"]
        Redis["Redis<br/>Cache + Queue"]
        RabbitMQ["RabbitMQ<br/>AMQP Dialer"]
...
<Info>**Coming Soon** — The Vaani Dialer is in development and available on the Dev instance for testing.</Info>
...
```mermaid
graph LR
    subgraph Ticker["⏱️ Ticker"]
        T["Campaign Tick<br/>emitted per interval"]
    end

    subgraph Consumers["AMQP Consumers"]
        H["🗂️ Hopper<br/>Fill prospect queue"]
        D["📞 Dialer<br/>Originate calls"]
        S["📅 Scheduler<br/>Handle scheduling"]
    end

    subgraph Data["Data Layer"]
        PG["PostgreSQL<br/>Campaigns, Prospects"]
        RMQ["RabbitMQ<br/>VHOSTS.DIALER"]
    end

    T -->|TickerExchange| H
    T -->|TickerExchange| D
    T -->|SchedulingExchange| S
...
- **Campaign fields** — `agent_id`, `workspace`, `email` link each campaign to a Vaani agent and workspace
